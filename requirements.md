...
- **時間割の学期分割とドラッグでの区間指定**
  - 各曜日・時限セルを「春学期 / 夏学期 / 秋学期 / 冬学期」の 4 つのレーンに縦分割して表示する。
  - それぞれのレーンは独立したドロップ領域となり、講義一覧の行をドラッグ＆ドロップして「この学期のこの曜日・この時限」に割り当てられる。
  - 学期情報は以下のドメインモデルで管理する:
    - `Term`: `'spring' | 'summer' | 'autumn' | 'winter'`
    - `TimetableEntry`:
      - `cellId: TimeCellId`（曜日＋時限）
      - `courseId: string`
      - `startTerm: Term`
      - `endTerm: Term`
  - ドロップした直後は `startTerm = endTerm = ドロップ先学期` の 1 学期区間として登録する。
  - 時間割セル内では、`startTerm`〜`endTerm` の範囲を 1 枚の縦長カードとして描画し、そのカードに対して上下端ハンドルを表示する。
    - 上端ハンドルをドラッグして他学期レーンの上でドロップすると `startTerm` を更新する（`startTerm` は `endTerm` を超えないよう `min` を取る）。
    - 下端ハンドルをドラッグして他学期レーンの上でドロップすると `endTerm` を更新する（`endTerm` は `startTerm` 未満にならないよう `max` を取る）。
  - 同じ `cellId` / `courseId` に対しては常に **1 件の `TimetableEntry` だけ** を持ち、`startTerm` / `endTerm` の区間で「どの学期にまたがっているか」を表現する。
    - これにより、春学期〜夏学期にまたがる 2 学期間の講義でも、1 つの講義として扱われる。
  - 単位計算は引き続き `courseId` と `TakenCourseState.status` のみを参照し、`startTerm` / `endTerm` や区間の長さには依存しない。
    - したがって、何学期にまたがっていても **その講義の `credits` は 1 回だけカウント** される（2 学期にまたがる 2 単位の講義を 4 単位として数えない）。
...
- **2026-03-16: shadcn/ui の Input コンポーネント未追加による Module not found**
  - 現象:
    - `Module not found: Can't resolve '@/components/ui/input'` というエラーが `app/page.tsx` の `import { Input } from "@/components/ui/input";` 行で発生。
  - 原因:
    - shadcn/ui の初期導入時に `input` コンポーネントを追加しておらず、`components/ui/input.tsx` が存在しなかった。
  - 対策:
    - `components/ui/input.tsx` を shadcn/ui の標準実装に倣って作成し、`cn` ヘルパーを用いたスタイル付き入力コンポーネントとして定義。
    - もしくは `npx shadcn-ui add input` を実行して追加する。
  - 再発防止ルール:
    - 新しい shadcn/ui コンポーネントを `import` するときは、事前に `npx shadcn-ui add <name>` を実行するか、既に `components/ui` にファイルが存在するかを確認する。

- **2026-03-16: `React is not defined` エラー（Client Component 内でのフック使用時）**
  - 現象:
    - `ReferenceError: React is not defined` が `CourseTable` コンポーネント内 (`app/page.tsx`) の  
      `const [query, setQuery] = React.useState("");` 行で発生。
  - 原因:
    - ファイル冒頭で `React` をインポートしておらず、JSX/フックから `React` 名前空間を参照できなかった。
    - `useState` を `React.useState` 形式で呼び出しているため、暗黙インポートにも頼れなかった。
  - 対策:
    - `app/page.tsx` の先頭に `import * as React from "react";` を追加し、`React.useState` を正しく解決できるようにした。
  - 再発防止ルール:
    - Client Component で `React.useState` / `React.useEffect` など名前空間経由でフックを使う場合は、必ず `import * as React from "react";` を追加する。
    - もしくは、`import { useState } from "react";` とし、コード側では `useState` を直接使うようにスタイルを統一する。

- **2026-03-16: `Switch is not defined` エラー（UI コンポーネントのインポート漏れ）**
  - 現象:
    - `ReferenceError: Switch is not defined` が `app/page.tsx` 内の `<Switch ... />` 行で発生。
  - 原因:
    - `components/ui/switch.tsx` を作成したものの、`app/page.tsx` で `import { Switch } from "@/components/ui/switch";` を追加し忘れていた。
  - 対策:
    - `app/page.tsx` の先頭で `Switch` を明示的にインポートするよう修正。
  - 再発防止ルール:
    - JSX に新しいコンポーネント名を書いたときは、同じファイルの先頭に対応する `import` があるか必ず確認する。

- **2026-03-16: 講義一覧と時間割カードの高さ・スクロール崩れ（Flexboxの誤用）**
  - 現象:
    - 講義一覧カード内の `ScrollArea` の高さが機能せず、時間割カードの長さに合わせて講義一覧カード自体が画面を大きくはみ出して伸びてしまった。
    - 中途半端に `flex-1` や `flex-col` を指定したが、特定の階層（特に `TabsContent` 周辺）で高さの制約が切れ、`ScrollArea` が無限に伸びてしまった。
  - 原因:
    - Flexbox 子要素のデフォルト動作である `min-height: auto`（コンテンツ以下に縮まない）の仕様により、中のコンテンツが大きい場合、親の `flex-1` 指定だけでは高さを制限できず、全体がコンテンツの高さまで押し広げられてしまう。
    - また、時間割カードが非常に長くなるため、`flex-row` の `stretch` 動作により講義一覧カードもそれに追従して無駄に長くなっていた。
  - 対策:
    - 画面全体の高さを基準にするため、親セクションに `h-[calc(100vh-14rem)] min-h-[600px]` を設定し、セクション自体の高さを画面内に固定。
    - 各カードおよびその中身のコンテナ（`Card`, `CardContent`, `Tabs`, `TabsContent` など）に `flex flex-col` と併せて **`min-h-0`** または `overflow-hidden` を付与し、子要素が親の高さを超えて押し広げないように制約を伝播させた。
    - 時間割カード側も `ScrollArea` で囲むことで、時間割表が伸びてもカード全体が伸びるのではなくカード内部でスクロールするようにした。
  - 再発防止ルール:
    - Tailwind / Flexbox で縦幅を制限して内部スクロール（`ScrollArea` 等）を実現したい場合は、親から子へと連なる **全ての Flex コンテナ階層** に `min-h-0` や `overflow-hidden` を設定し、高さ制約が途切れないようにすること。
    - 特定の要素の高さに引きずられないようにするには、大枠のコンテナに `h-[...]` 等で明確な上限を設ける設計を基本とする。
